...
fn next_position(
    ...
    match dir {
        Direction::Up => Ok((r.checked_sub(1).ok_or_else(|| anyhow!("row underflow"))?, c)),
        Direction::Left => Ok((r, c.checked_sub(1).ok_or_else(|| anyhow!("col underflow"))?)),
        Direction::Right => {
            ensure!(c + 1 < cols, "col overflow");
            Ok((r, c + 1))
        }
        Direction::Down => {
            ensure!(r + 1 < rows, "row overflow");
            Ok((r + 1, c))
        }
        Direction::UpLeft => Ok((
            r.checked_sub(1).ok_or_else(|| anyhow!("row underflow"))?,
            c.checked_sub(1).ok_or_else(|| anyhow!("col underflow"))?,
        )),
        Direction::UpRight => {
            ensure!(c + 1 < cols, "col overflow");
            Ok((
...
fn compute_direction_products(input: &[Vec<i32>], start: (usize, usize)) -> Vec<i32> {
    let dims = (input.len(), input[0].len());
    Direction::iter()
        .filter_map(|dir| {
            let positions: Vec<_> =
                std::iter::successors(Some(start), |&pos| next_position(dims, pos, &dir).ok())
                    .take(4)
                    .collect();

            (positions.len() == 4).then(|| positions.into_iter().map(|(r, c)| input[r][c]).product())
        })
        .collect()
}
...
fn main() {
    run(0, || {
        ...
        (0..input.len())
            .cartesian_product(0..input.first().unwrap().len())
            .flat_map(|coords| compute_direction_products(&input, coords))
            .max()
            .unwrap()
    });
}
